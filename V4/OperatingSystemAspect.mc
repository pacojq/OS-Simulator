...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
 ...
 ComputerSystem_DebugMessage(142, 'm', PID,
 ...
 OperatingSystem_ShowTime('m');
 ComputerSystem_DebugMessage(143, 'm', partition,
...
void OperatingSystem_ReleaseMainMemory(int PID) {
 ...
 OperatingSystem_ShowTime('m');
 ComputerSystem_DebugMessage(145, 'm', partition,
  partitionsTable[partition].initAddress, partitionsTable[partition].size,
...
int OperatingSystem_ShortTermScheduler() {
 ...
 while(selectedProcess == -1 && queue < 2);
...
void OperatingSystem_HandleClockInterrupt() {
 ...
 if (awakenProcesses + createdProcesses > 0) {
  ...
  if (Heap_compare(newPID, executingProcessID, 1) > 0 ||
   ...
   Heap_add(newPID, readyToRunQueue[queue], 1,
    &numberOfReadyToRunProcesses[queue], 4);
  }
 }
}
...
void OperatingSystem_MoveProcessToState(int PID, int newState) {
 ...
 OperatingSystem_ShowTime('p');
 if (newState == NEW)
  ComputerSystem_DebugMessage(111, 'p', PID, statesNames[NEW]);
 else
...
void OperatingSystem_PrintReadyToRunQueue() {
 ...
 for (q = 0; q < 2; q ++) {
  ...
  int msg = -1;
  switch (q) {
   case 0: msg = 112; break;
   case 1: msg = 113; break;
  }
  if (msg != -1)
