...
void Processor_InitializeInterruptVectorTable(int interruptVectorInitialAddress) {
	...
	interruptVectorTable[CLOCK_INT_BIT]=interruptVectorInitialAddress+4; // CLOCK_INT_BIT = 9
}
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (registerIR_CPU.operationCode) {
		...
		// Instruction MEMADD 	(MEMADD = READ + ADD)
		case 'm':
			...
			// Sum registerIR_CPU.operand1 and the value in memory
		...
		case 'h':
			...
			else Processor_RaiseException(INVALIDPROCESSORMODE);
			break;
		...
		case 'o': // Make a operating system routine in entry point indicated by operand1			
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)) {			
				...
				ComputerSystem_DebugMessage(130, HARDWARE, OperatingSystem_GetExecutingProcessID(),
	...
	int i;
		...
		for (i=0;i<INTERRUPTTYPES;i++)
			...
			if (Processor_GetInterruptLineStatus(i)) {
				...
				// Activate protected excution mode and masked bit
