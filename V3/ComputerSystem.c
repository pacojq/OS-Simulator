	...
	for(i = 1; i < PROGRAMSMAXNUMBER; i ++) {
		if (programList[i] != NULL)
			ComputerSystem_DebugMessage(102, INIT,
				programList[i]->executableName, programList[i]->arrivalTime);
	}
}
...
int ComputerSystem_GetFromArrivalTimeQueue() {
	...
	selectedProcess = Heap_getFirst(arrivalTimeQueue, numberOfProgramsInArrivalTimeQueue);
...
int ComputerSystem_ExtractFromArrivalTimeQueue() {  
	int selectedProcess=NOPROCESS;
	selectedProcess = Heap_poll(arrivalTimeQueue, QUEUE_ARRIVAL, &numberOfProgramsInArrivalTimeQueue);
	return selectedProcess; 
}
